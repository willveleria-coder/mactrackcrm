  ...
  function getStatusInfo(status) {
    const statusMap = {
      pending: {
        ...
        icon: "📋",
        color: "bg-yellow-500",
        description: "Your order has been received and is being prepared for pickup."
      },
      active: {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-gray-600 text-lg font-semibold">Loading order details...</div>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
          <div className="flex items-center justify-center gap-3">
            ...
            />
            <div className="text-center">
              <h1 className="text-2xl sm:text-3xl font-black text-red-600">Mac Track</h1>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="bg-white rounded-2xl shadow-2xl border-2 border-gray-100 p-6 sm:p-8 mb-6">
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl font-black text-gray-900 mb-2">{statusInfo.label}</h2>
          ...
          <div className="space-y-4">
            <div className={`flex items-start gap-4 p-4 rounded-xl ${order.status === 'pending' || order.status === 'active' || order.status === 'delivered' ? 'bg-green-50 border-2 border-green-200' : 'bg-gray-50'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${order.status === 'pending' || order.status === 'active' || order.status === 'delivered' ? 'bg-green-500 text-white' : 'bg-gray-300'}`}>
                ✓
              </div>
            ...
            {driver && (order.status === 'active' || order.status === 'delivered') && (
              <div className={`flex items-start gap-4 p-4 rounded-xl ${order.status === 'active' || order.status === 'delivered' ? 'bg-blue-50 border-2 border-blue-200' : 'bg-gray-50'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${order.status === 'active' || order.status === 'delivered' ? 'bg-blue-500 text-white' : 'bg-gray-300'}`}>
                  ✓
                </div>
                <div className="flex-1">
                  <p className="font-bold text-gray-900">Out for Delivery</p>
                  <p className="text-sm text-gray-600">Driver: {driver.name}</p>
                  <p className="text-sm text-gray-600">Vehicle: {driver.vehicle_type} - {driver.vehicle_registration}</p>
                </div>
              </div>
            )}
            ...
            {order.status === 'delivered' && (
              <div className="flex items-start gap-4 p-4 rounded-xl bg-green-50 border-2 border-green-200">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600">{order.delivered_at ? new Date(order.delivered_at).toLocaleString() : 'Completed'}</p>
                </div>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
            ...
            <p className="text-gray-700 leading-relaxed">{order.pickup_address}</p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
            ...
            <p className="text-gray-700 leading-relaxed">{order.dropoff_address}</p>
          </div>
        </div>

        {/* Parcel Details */}
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-6">
          <h3 className="text-lg font-bold text-gray-900 mb-4">📦 Parcel Information</h3>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div>
              <p className="text-xs text-gray-500 mb-1">Size</p>
              <p className="text-sm font-bold text-gray-900 capitalize">{order.parcel_size}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">Weight</p>
              <p className="text-sm font-bold text-gray-900">{order.parcel_weight} kg</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">Service</p>
              <p className="text-sm font-bold text-gray-900 capitalize">{order.service_type.replace('_', ' ')}</p>
            </div>
            <div>
        ...
        {/* Driver Contact (only if active or delivered) */}
        {driver && (order.status === 'active' || order.status === 'delivered') && (
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600 capitalize">{driver.vehicle_type}</p>
                <p className="text-sm text-gray-600">{driver.vehicle_registration}</p>
              </div>
              {order.status === 'active' && (
                <a 
        ...
        {order.notes && (
          <div className="bg-yellow-50 rounded-2xl border-2 border-yellow-200 p-6 mb-6">
            <h3 className="text-lg font-bold text-gray-900 mb-3">📝 Delivery Instructions</h3>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          <button
            onClick={() => window.open(`https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(order.pickup_address)}&destination=${encodeURIComponent(order.dropoff_address)}`, '_blank')}
