...
export default function ParcelTypesPage() {
  ...
  async function loadParcelTypes() {
    try {
      ...
      console.error("Error loading parcel types:", error);
      setMessage("❌ Failed to load parcel types");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (editingType) {
        ...
        if (error) throw error;
        setMessage("✅ Parcel type updated successfully!");
      } else {
        // Create new
        const { error } = await supabase
          .from("parcel_types")
        ...
        if (error) throw error;
        setMessage("✅ Parcel type created successfully!");
      }

      await loadParcelTypes();
      setTimeout(() => {
      ...
      setMessage("❌ Failed to save parcel type");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id) {
    if (!confirm("Are you sure you want to delete this parcel type?")) return;
    ...
    try {
      ...
      setMessage("✅ Parcel type deleted successfully!");
      await loadParcelTypes();
      setTimeout(() => setMessage(''), 3000);
    } catch (error) {
      console.error("Error deleting parcel type:", error);
      setMessage("❌ Failed to delete parcel type");
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4 sm:gap-6">
              ...
              <Link href="/admin/parcel-types" className="text-sm font-semibold text-red-600 border-b-2 border-red-600">
                Parcel Types
              </Link>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-3xl font-bold text-gray-900 mb-2">📦 Parcel Types</h2>
            <p className="text-gray-600">Manage available parcel types and pricing</p>
          </div>
          <button
            ...
            <div key={type.id} className={`bg-white rounded-2xl shadow-lg border-2 p-6 ${type.is_active ? 'border-gray-100' : 'border-red-200 opacity-60'}`}>
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  {!type.is_active && (
                    <span className="inline-block px-2 py-1 bg-red-100 text-red-700 text-xs font-bold rounded-full mt-1">
                      INACTIVE
                    </span>
                  )}
                </div>
              ...
              <div className="space-y-2 mb-4">
                {type.max_weight && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-semibold">{type.max_weight} kg</span>
                  </div>
                )}
                {(type.max_length || type.max_width || type.max_height) && (
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">Max Dimensions:</span>
                    <span className="font-semibold">
                      {type.max_length || '∞'} × {type.max_width || '∞'} × {type.max_height || '∞'} cm
                    </span>
                  </div>
                )}
              </div>
        ...
        {parcelTypes.length === 0 && (
          <div className="text-center py-16 bg-white rounded-2xl shadow-lg border border-gray-100">
            ...
            <p className="text-gray-500 text-lg font-semibold mb-4">No parcel types yet</p>
            <button
              onClick={openAddModal}
              className="px-6 py-3 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition"
            >
              Add Your First Parcel Type
            </button>
          </div>
        )}
      </main>
          ...
          <div className="fixed top-8 left-1/2 transform -translate-x-1/2 bg-white rounded-2xl shadow-2xl p-6 sm:p-8 z-50 w-11/12 max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-6">
              <h3 className="text-2xl font-bold text-gray-900">
                {editingType ? 'Edit Parcel Type' : 'Add Parcel Type'}
              </h3>
              <button 
                onClick={() => setShowAddModal(false)}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    required
                    placeholder="e.g., Box, Satchel, Tube"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleInputChange}
                    rows={2}
                    placeholder="Brief description of this parcel type"
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">
                    Price Multiplier * (Base price × this value)
                  </label>
                  <input
                    type="number"
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Example: 1.0 = standard price, 1.5 = 50% more expensive, 0.8 = 20% cheaper
                  </p>
                </div>
