...
async function handleBulkStatusUpdate() {
  ...
  if (order.custom_eta) {
    ...
    etaString = customDate.toLocaleString("en-AU", { day: "numeric", month: "short", hour: "numeric", minute: "2-digit" }) + ' (Custom)';
  } else {
...
<div class="label-wrapper">
  ...
  <div class="label">
    <div class="header">
      ...
      <div><div class="order-id-label">ORDER</div><div class="order-id-value">${orderNumber}</div></div>
    </div>
    <div class="qr-row">
    ...
    <div class="customer-row">
      ...
      <div style="text-align:right"><div class="date-label">ETA</div><div class="date-value ${order.custom_eta ? 'eta-custom' : ''}">${etaString}</div></div>
    </div>
    <div class="footer">
...
{visibleColumns.pickup && (
                        ...
                        {visibleColumns.eta && (
  <td className="px-6 py-4">
    <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${isCustomEta ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>
      {isCustomEta && '✏️ '}{etaString}
    </span>
  </td>
)}
...
>
  {order.payment_status === 'paid' ? '✅ Paid' : '💳 Send Payment'}
</button>
                            </div>
                          </td>
                        )}
