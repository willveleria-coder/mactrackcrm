              ...
              {loading ? (
                ...
                conversations.map((conv) => (
                  <button
                    ...
                    className={`w-full p-4 text-left border-b hover:bg-gray-100 transition ${
                      selected?.id === conv.id ? "bg-red-50 border-l-4 border-l-red-600" : ""
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white ${
                        conv.participant_type === "client" ? "bg-blue-500" : "bg-green-500"
                      }`}>
              ...
              <>
                <div className="p-4 border-b bg-white flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white text-xl ${
                    selected.participant_type === "client" ? "bg-blue-500" : "bg-green-500"
                  }`}>
                    {selected.participant_type === "client" ? "👤" : "🚗"}
                  </div>
                  <div>
                    <p className="font-bold text-lg">{selected.participant?.name}</p>
                    <p className="text-sm text-gray-500 capitalize">{selected.participant_type} • {selected.participant?.email}</p>
                  </div>
                </div>
                  ...
                  {messages.map((msg) => (
                    <div key={msg.id} className={`flex ${msg.sender_type === "admin" ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[70%] p-3 rounded-2xl ${
                        ...
                        <p>{msg.message}</p>
                        <p className={`text-xs mt-1 ${msg.sender_type === "admin" ? "text-red-200" : "text-gray-400"}`}>
                ...
                <form onSubmit={sendMessage} className="p-4 border-t bg-white">
                  <div className="flex gap-2">
                    ...
                    />
                    <button type="submit" className="px-6 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700">
                      Send
                    </button>
                  </div>
                </form>
